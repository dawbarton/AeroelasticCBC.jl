...
const BASE_PARS =
    ComponentArray(; U = 15, b = 0.15, a = -0.5, ρ = 1.204, m_w = 5.3, m_T = 16.9)
...
function unsteady_model!(du, u, p, t)
    ...
    # Build linear terms
    M⁻¹ = @SMatrix [
        (true+(((b*m_w*x_α-a*π*ρ*(b^3))/(m_T+π*ρ*(b^2)))*(b*m_w*x_α-a*π*ρ*(b^3)))/(I_α+(-((b * m_w * x_α - a * π * ρ * (b^3))^2))/(m_T+π*ρ*(b^2))+π*ρ*(b^4)*(0.125+a^2)))/(m_T+π*ρ*(b^2)) (-((b * m_w * x_α - a * π * ρ * (b^3)) / (I_α + (-((b * m_w * x_α - a * π * ρ * (b^3))^2)) / (m_T + π * ρ * (b^2)) + π * ρ * (b^4) * (0.125 + a^2))))/(m_T+π*ρ*(b^2)) 0
    ...
    D = @SMatrix [
        c_h+2*π*ρ*b*U*(c₀-c₁-c₃) (1+(c₀-c₁-c₃)*(1-2a))*π*ρ*b^2*U 2π*ρ*U^2*b*(c₁*c₂+c₃*c₄)
        -2π*(a+1/2)*ρ*b^2*(c₀-c₁-c₃)*U c_α+(1/2-a)*(1-(c₀-c₁-c₃)*(1+2a))*π*ρ*b^3*U -2π*ρ*b^2*U^2*(a+1/2)*(c₁*c₂+c₃*c₄)
        -1/b a-1/2 (c₂+c₄)*U/b
    ]

    K = @SMatrix [
        k_h 2π*ρ*b*U^2*(c₀-c₁-c₃) 2π*ρ*U^3*c₂*c₄*(c₁+c₃)
