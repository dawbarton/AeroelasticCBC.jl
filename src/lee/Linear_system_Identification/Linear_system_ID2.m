...
% First run linear_system_ID.m
load('Measured_data2','data')
...
%Free parameters: c__alpha, w_a, w_h
[c__alpha,w_a,w_h]=Lin_parameter_ID(data,b,a,m,m__T,w_a,w_h,x__alpha,r_a,c__alpha,c__h,Ts,fs,sen,t_end);
...
function [c__alpha,w_a,w_h] = Lin_parameter_ID(data,b,a,m,m__T,w_a,w_h,x__alpha,r_a,c__alpha,c__h,Ts,fs,sen,t_end)
h=data(2,:);
p=data(1,:);

h=h*sen; % Multiplying sensitivity of the measurement
zh=mean(h(end-300:end));  % Zero position of the displament (heave)
zp=mean(p(end-300:end)); % Zero position of the displament (angle)
...
w_h=linear_model.Structure.Parameters(6).Value;
end
